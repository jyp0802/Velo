...
if ($uid!="" and $bid!=""){
  ...
  if($result){
    ...
    if(mysqli_fetch_row($result)[0] != '0'){
      ...
      //$sql="UPDATE Bike SET bike_status = 0 WHERE id='$bid';";
      $sql="UPDATE Bike SET bike_status = 0, bike_x = '$bike_x', bike_y = '$bike_y' WHERE id='$bid';";
      $result=mysqli_query($link,$sql);

      if($result) {

        $sql="DELETE from Borrow where borrow_uid ='$uid' and borrow_bid = '$bid';";
        $result=mysqli_query($link,$sql);
        ...
        if($result) {
          ...
          echo "SQL error : 1"; // borrow delete에서 오류
