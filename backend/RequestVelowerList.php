...
if ($uid !=""){
  $sql="SELECT u.id, user_name, bike_type FROM User u, Velow v, Owner o, Bike b WHERE o.owner_uid = u.id and o.owner_bid = b.id and u.id=velower_id and velowee_id='$uid'  UNION  select id, user_name, bike_type from (SELECT id, user_name, owner_uid as bike_type, user_email FROM User u LEFT JOIN Owner o on o.owner_uid = u.id) as T, velow where bike_type is null and id=velower_id and velowee_id='$uid';";
